...
class TestUser(OperationTest):
    ...
    def test_create_iot_token(self,client):
        ...
        url = self.prefix + '/create-iot-token'
    ...
    def test_validation_get_multiple_user(self,client):
        url = self.prefix + '/get-multiple-user'
        # field required
        response = client.post(url,json={})
        assert response.status_code == 422
        for x in response.json()['detail']:
    ...
    @pytest.mark.asyncio
    async def test_get_multiple_user(self,async_client):
        url = self.prefix + '/get-multiple-user'

        user_id = await self.get_user_id(self.account_1['email'])
        # user not found
        response = await async_client.post(url,json={'list_id': ['9' * 8]})
        assert response.status_code == 200
        assert response.json() == []

        response = await async_client.post(url,json={'list_id': [str(user_id)]})
        assert response.status_code == 200
        # check data exists and type data
        assert type(response.json()[0]['id']) == str
        assert type(response.json()[0]['username']) == str
        assert type(response.json()[0]['avatar']) == str
