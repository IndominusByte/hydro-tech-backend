...
from schemas.users.UserSchema import (
    ...
    UserLogin, UserResetPassword,
    UserData, UserMultiple, UserDataChat
)
...
@router.post('/register',status_code=201,
    responses={
        201: {
            ...
            "content": {"application/json":{"example": {"detail":"Check your email to activated your account."}}}
        },
        400: {
...
async def register(request: Request, user: UserRegister, background_tasks: BackgroundTasks):
    ...
    return {"detail":"Check your email to activated your account."}
...
@router.post('/create-iot-token',
    responses={
        200: {
            "description": "Successful Response",
            "content": {"application/json": {"example": {"token": "string"}}}
        }
    }
)
async def create_iot_token(authorize: AuthJWT = Depends()):
    authorize.fresh_jwt_required()

    user_id = int(authorize.get_jwt_subject())
    if user := await UserFetch.filter_by_id(user_id):
        token = authorize.create_access_token(subject=str(user['id']),expires_time=False)
        return {'token': token}
...
@router.post('/get-multiple-user',response_model=List[UserDataChat])
async def get_multiple_user(user_data: UserMultiple):
    return await UserFetch.get_multiple_user(user_data.list_id)
