...
def validate_multiple_upload_images(
    ...
    for index, image in enumerate(images, 1):
        ...
        try:
            ...
            msg = "Cannot identify the image at index {index}."
            raise HTTPException(status_code=422,detail=msg.format(**{'index': index}))

        # validation size image
        size = image.file
        size.seek(0,os.SEEK_END)
        if size.tell() > max_file_size_mb:
            msg_size = "An image at index {index} cannot greater than {max_file_size} Mb."
            ctx = {'index': index, 'max_file_size': max_file_size}
            raise HTTPException(status_code=413,detail=msg_size.format(**ctx))
        size.seek(0)
    ...
    if min_file_in_list and len(images) < min_file_in_list:
        msg_min_file = "At least {min_file_in_list} image must be upload."
        ctx = {'min_file_in_list': min_file_in_list}
        raise HTTPException(status_code=422,detail=msg_min_file.format(**ctx))

    if max_file_in_list and len(images) > max_file_in_list:
        msg_max_file = "Maximal {max_file_in_list} images to be upload."
        ctx = {'max_file_in_list': max_file_in_list}
        raise HTTPException(status_code=422,detail=msg_max_file.format(**ctx))
...
def validate_single_upload_image(
    ...
    size.seek(0,os.SEEK_END)
    if size.tell() > max_file_size_mb:
        msg_size = "An image cannot greater than {max_file_size} Mb."
